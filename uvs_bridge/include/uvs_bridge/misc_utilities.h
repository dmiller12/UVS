...
std::vector<std::string> get_directory_files(std::string directory_path)
{
    ...
    try {
        if (boost::filesystem::exists(p)) {
            ...
            ROS_WARN_STREAM("Error: Directory does not exist" << directory_path << std::endl);
        }
...
std::vector<Eigen::VectorXd> read_joint_positions_from_file(int dof)
{
    ...
    std::string prefix = "/home/hulk/AssistiveRobotics/ros_workspace/src/wam_srvs/log_data/waypoints/";
    std::ifstream f;
    while (true) {
...
std::vector<Eigen::VectorXd> read_waypoints_from_file(int dof, std::string prefix)
{
    std::vector<Eigen::VectorXd> joints;
    std::string line;
    std::istringstream iss;
